...
class ModbusScannerBase(ABC):
    ...
    def _build_base_ui(self):
        ...
        tk.Button(bot, text="Export Results / Log", bg="blue", fg="white", command=self._export_data).pack(side="left", padx=2)

        wf = tk.Frame(bot, bd=1, relief=tk.GROOVE)
        wf.pack(side="left", padx=10, fill="y")
        
        self.chk_record = tk.Checkbutton(wf, text="Record Data", variable=self.record_enabled, command=self._toggle_record)
    ...
    def start_scan(self):
        ...
        # Clear history if not recording
    ...
    def _ui_tick(self):
        ...
        # 2. Record History (Decoded Values)
    ...
    def _capture_decoded_snapshot(self):
        """Captures the current values DECODED into their proper types (float, int, etc)"""
    ...
    def _toggle_record(self):
        pass # Logic handled in _ui_tick
    ...
    def _export_data(self):
        ...
        if has_log and HAS_XLSXWRITER:
            ans = messagebox.askyesno("Export", f"You have {len(self._history_data)} recorded samples.\n\n"
                                                "Yes = Export Trend Log (Excel with Chart)\n"
                                                "No  = Export Current Snapshot (CSV)")
            if ans: export_mode = "log"
        elif has_log and not HAS_XLSXWRITER:
             messagebox.showinfo("Export info", "Log data exists, but 'xlsxwriter' is not installed.\nExporting snapshot CSV only.")

        if export_mode == "snapshot":
            self._download_results_csv()
        else:
            self._download_results_excel_trend()
    ...
    def _download_results_csv(self):
        ...
        try:
            ...
            messagebox.showinfo("Export", f"Snapshot saved to {path}")
        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def _download_results_excel_trend(self):
        ...
        try:
            ...
            # Determine ALL unique labels seen in history (in case scan range changed)
            all_labels = set()
            for ts, snap in self._history_data:
                all_labels.update(snap.keys())
            
            # Sort labels nicely (numerical sort attempt)
            def sort_key(s):
            ...
            # Graph first 15 valid columns
            count_graphed = 0
            for i, lbl in enumerate(sorted_labels):
                col_idx = i + 1 
                # Check if data exists in this col
            ...
            chart.set_title ({'name': 'Modbus Trend Log'})
            ...
            messagebox.showinfo("Export", f"Trend Log with Chart saved to {path}")
